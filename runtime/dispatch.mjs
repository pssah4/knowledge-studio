...
async function executeAction(root,request,{bindings={},extract,editorHTML=null}={}){
 ...
 if(action==='query'||action==='graph'||action==='readiness'||action.startsWith('shadow.')){
   ...
   if(action==='query'||action.startsWith('shadow.')){
    ...
    try{
     ...
     const result=await editorCitations(root,p,contexts,await query(wikis,args,{shadow}));if(inaccessible.length){result.findings.push(...inaccessible);result.complete=false;}return result;
    }finally{db?.close();}
   }
   if(action==='readiness'){
    ...
    const originals=[];
    for(const c of contexts)for(const item of knowledge.source_pages.filter(p=>p.wiki===c.wiki.id)){
 ...
 if(action==='sync'){const result=await sync.run(options);await exportGraph(store);if(c.canPublish)await exportGraph(c.remote);return result;}
 if(action==='conflict.resolve'){await sync.resolve(options,args.conflict,args.text,args.author,args.message);return sync.run(options);}
 ...
 if(action==='review.respond'){
   ...
   const event=reviews.reply(parent,args.choice,args.author,seen.text,args.text??seen.text,args.message);event.page=args.page;await reviews.append(handle(store),event);return {recorded:true,event};
 }
}
