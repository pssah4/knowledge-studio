...
async function request(q,retry=true){
 let response;try{response=await fetch('/api',{method:'POST',headers:{'Content-Type':'application/json',Authorization:'Bearer '+token},body:JSON.stringify(q)});}catch{throw Error('Der Editor-Dienst ist nicht erreichbar. Öffne „Editor starten“ im Projekt oder lass den Agent editor.start ausführen. Dein Entwurf bleibt erhalten.');}
...
async function confirm(){
 ...
 confirmation=(async()=>{for(;;){const c=await request({op:'catalog'},false);if(c.granted)return c;
  const accepted=await new Promise((resolve,reject)=>{const d=document.createElement('dialog');d.className='ws-dialog';const header=document.createElement('header'),content=document.createElement('div'),foot=document.createElement('footer');header.className='ws-dialog-head';content.className='ws-dialog-content';foot.className='ws-dialog-foot';d.append(header,content,foot);const h=document.createElement('h2');h.textContent='Zugriff auf deine verbundenen Ordner';header.append(h);const intro=document.createElement('p');intro.textContent='Bestätige die bereits eingerichteten Ordner. Der Editor verwaltet die Arbeitskopien für dich; eine erneute Ordnerauswahl ist nicht nötig.';content.append(intro);
   const copies=document.createElement('details'),summary=document.createElement('summary');summary.textContent='Arbeitskopien und Projekteinstellungen';copies.append(summary);
   for(const f of c.folders){const p=document.createElement('p'),name=document.createElement('strong'),location=document.createElement('small');name.textContent=f.label+' · '+(f.writable?'Lesen und Schreiben':'Nur lesen');location.textContent=f.root||f.error||'Nicht verbunden';location.style.display='block';location.style.overflowWrap='anywhere';p.append(name,location);(f.kind==='work'||f.kind==='project'?copies:content).append(p);}content.append(copies);
   const button=document.createElement('button');button.textContent='Zugriff erlauben';button.className='ws-primary';button.onclick=async()=>{button.disabled=true;try{await request({op:'grant',fingerprint:c.fingerprint},false);d.close();d.remove();resolve(true);}catch(error){if(error.code==='stale'){d.close();d.remove();resolve(false);return;}button.disabled=false;const p=document.createElement('p');p.textContent=error.message;content.append(p);}};
   const cancel=()=>{d.close();d.remove();reject(Error('Zugriff nicht bestätigt. Du kannst ihn jederzeit erlauben.'));};const back=document.createElement('button');back.textContent='Abbrechen';back.onclick=cancel;d.addEventListener('cancel',cancel);foot.append(button,back);document.body.append(d);d.showModal();
